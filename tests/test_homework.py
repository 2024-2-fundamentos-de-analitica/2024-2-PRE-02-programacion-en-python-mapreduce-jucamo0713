...
def test_01():
    ...
    wc.run_job(
        os.path.join("files","input"),
        os.path.join("files","output"),
    )

    #
    # Retorna error si la carpeta output/ no existe
    if not os.path.exists(os.path.join("files","output")):
        raise Exception("Output directory does not exist")

    #
    # Retorna error si el archivo "_SUCCESS" no existe en la
    # carpeta output/
    if not os.path.exists(os.path.join("files","output","_SUCCESS")):
    ...
    with open(os.path.join("files","output","part-00000"), "r", encoding="utf-8") as f:
        lines = f.readlines()
        result = {}
        for line in lines:
